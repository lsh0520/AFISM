...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AFISM.")
    ...
    parser.add_argument('--adv_epoch', type=int, default= 1000,
                        help='Add APR in epoch X, when adv_epoch is 0, it\'s equivalent to pure AFISM.\n '
                             'And when adv_epoch is larger than epochs, it\'s equivalent to pure FISM model. ')
...
class FISM:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.delta_c1 = tf.Variable(tf.zeros(shape=[self.num_items, self.embedding_size]),
                                           name='delta_c1', dtype=tf.float32, trainable=False)
            self.delta_c2 = tf.constant(0.0, tf.float32, [1, self.embedding_size], name='c2' )
            self.delta_P = tf.concat([self.delta_c1,self.delta_c2], 0, name='delta_Q_')
            
            self.delta_Q = tf.Variable(tf.zeros(shape=[self.num_items, self.embedding_size]),
                                           name='delta_Q', dtype=tf.float32, trainable=False)
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            self.bias_i = tf.nn.embedding_lookup(self.bias, item_input)

            return (tf.expand_dims(tf.reduce_sum(self.embedding_p*self.embedding_q, 1),1) + self.bias_i), self.embedding_p, self.embedding_q 
    ...
    def _create_inference_adv(self, item_input):
        with tf.name_scope("inference_adv"):
            ...
            return (tf.expand_dims(tf.reduce_sum(self.P_plus_delta*self.Q_plus_delta, 1),1) + self.bias_i) , self.embedding_p, self.embedding_q  # (b, embedding_size) * (embedding_size, 1)
    ...
    def _create_adversarial(self):
        with tf.name_scope("adversarial"):
            ...
            # generate the adversarial weights by gradient-based method
                ...
                self.grad_Q_, self.grad_Q = tf.gradients(self.loss, [self.c1, self.embedding_Q])

                # convert the IndexedSlice Data to Dense Tensor
                self.grad_Q__dense = tf.stop_gradient(self.grad_Q_)
                self.grad_Q_dense = tf.stop_gradient(self.grad_Q)
        
                # normalization: new_grad = (grad / |grad|) * eps
                self.update_P = self.delta_c1.assign(tf.nn.l2_normalize(self.grad_Q__dense, 1) * self.eps)
...
def training(model, dataset, args, epoch_start, epoch_end, time_stamp):  # saver is an object to save pq
    with tf.Session() as sess:
        ...
        if args.adver:
            ...
            ckpt_restore_path = "Pretrain/%s/FISM_BPR/embed_%d/%s/" % (args.dataset, args.embed_size, time_stamp)
        else:
            ckpt_save_path = "Pretrain/%s/FISM_BPR/embed_%d/%s/" % (args.dataset, args.embed_size, time_stamp)
            ckpt_restore_path = 0 if args.restore is None else "Pretrain/%s/FISM_BPR/embed_%d/%s/" % (args.dataset, args.embed_size, args.restore)
        ...
        saver_ckpt = tf.train.Saver({'embedding_Q_': model.c1, 'embedding_Q': model.embedding_Q, 'bias': model.bias})
...
def output_evaluate(model, sess, dataset, train_batches, eval_feed_dicts, epoch_count, batch_time, train_time, prev_acc,
    ...
    # check embedding
    embedding_Q_, embedding_Q = sess.run([model.embedding_Q_, model.embedding_Q])
    ...
    res = "Epoch %d [%.1fs + %.1fs]: HR = %.4f, NDCG = %.4f ACC = %.4f ACC_adv = %.4f [%.1fs], |P|=%.2f, |Q|=%.2f" % \
          (epoch_count, batch_time, train_time, hr, ndcg, prev_acc,
           post_acc, eval_time, np.linalg.norm(embedding_Q_), np.linalg.norm(embedding_Q))
...
if __name__ == '__main__':
    ...
    args.adver = 0
    # initialize FISM_BPR models
    FISM_BPR = FISM(dataset.num_users, dataset.num_items, args)
    FISM_BPR.build_graph()

    print "Initialize FISM_BPR"

    # start training
    training(FISM_BPR, dataset, args, epoch_start=0, epoch_end=args.adv_epoch-1, time_stamp=time_stamp)

    args.adver = 1
    # instialize AFISM model
    AFISM = FISM(dataset.num_users, dataset.num_items, args)
    AFISM.build_graph()

    print "Initialize AFISM"

    # start training
    training(AFISM, dataset, args, epoch_start=args.adv_epoch, epoch_end=args.epochs, time_stamp=time_stamp)
